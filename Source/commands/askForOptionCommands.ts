...
export async function registerAskForChoiceCommand(
	context: ExtensionContext,
): Promise<void> {
	context.subscriptions.push(
		commands.registerCommand(
			JavaTestRunnerCommands.ASK_CLIENT_FOR_CHOICE,
			async (
				placeHolder: string,
				choices: IOption[],
				canPickMany: boolean,
				...
				try {
					result = await new Promise<string[] | undefined>(
						(resolve: (value: string[] | undefined) => void) => {
							const quickPick: QuickPick<IOption> =
								window.createQuickPick<IOption>();
							// if all the items are advanced item, show them directly
							let showAdvancedItem: boolean =
 ...
 * @param context
 */
export async function registerAskForInputCommand(
	context: ExtensionContext,
): Promise<void> {
	context.subscriptions.push(
		commands.registerCommand(
			JavaTestRunnerCommands.ASK_CLIENT_FOR_INPUT,
			async (prompt: string, value: string) => {
