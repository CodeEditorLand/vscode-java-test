...
export async function generateTests(
	...
	const edit: WorkspaceEdit = converter.asWorkspaceEdit(
		await askServerToGenerateTests(uri, cursorOffset),
	);
...
async function askServerToGenerateTests(
	uri: Uri,
	cursorOffset: number,
): Promise<any> {
	return await executeJavaLanguageServerCommand<any>(
		JavaTestRunnerDelegateCommands.GENERATE_TESTS,
		uri.toString(),
		cursorOffset,
	);
}
