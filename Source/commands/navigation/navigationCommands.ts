...
import {
	...
	JavaTestRunnerDelegateCommands,
	VSCodeCommands,
} from "../../constants";
...
export async function navigateToTestOrTarget(gotoTest: boolean): Promise<void> {
	...
	if (!result?.items?.length) {
		...
		const sortedResults: ITestNavigationItem[] = result.items.sort(
			(a: ITestNavigationItem, b: ITestNavigationItem) => {
				if (a.outOfBelongingProject && !b.outOfBelongingProject) {
					return Number.MAX_SAFE_INTEGER;
				} else if (
					!a.outOfBelongingProject &&
					b.outOfBelongingProject
				) {
...
async function searchTestOrTarget(
	uri: string,
	gotoTest: boolean,
): Promise<ITestNavigationResult | undefined> {
	return await executeJavaLanguageServerCommand<
		ITestNavigationResult | undefined
