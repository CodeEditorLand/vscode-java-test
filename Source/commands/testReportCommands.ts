...
export async function openStackTrace(
	...
	const uri: string = await resolveStackTraceLocation(
		trace,
...
async function resolveStackTraceLocation(
	trace: string,
	projectNames: string[],
): Promise<string> {
	return (
		(await executeJavaLanguageServerCommand<string>(
			JavaLanguageServerCommands.RESOLVE_STACKTRACE_LOCATION,
			trace,
			projectNames,
		)) || ""
	);
