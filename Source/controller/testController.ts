...
import {
	createTestItem,
	findDirectTestChildrenForClass,
	findTestPackagesAndTypes,
	findTestTypesAndMethods,
	loadJavaProjects,
	resolvePath,
	synchronizeItemsRecursively,
	updateItemForDocumentWithDebounce,
} from "./utils";
...
export const loadChildren: (item: TestItem, token?: CancellationToken) => any =
	instrumentOperation(
		"java.test.explorer.loadChildren",
		async (
			_operationId: string,
			item: TestItem,
...
async function runHandler(
	...
	await runTests(request, {
		token,
		isDebug: !!request.profile?.label.includes("Debug"),
	});
}

export const runTests: (request: TestRunRequest, option: IRunOption) => any =
	instrumentOperation(
		"java.test.runTests",
		async (
			operationId: string,
			...
			const testItems: TestItem[] = await new Promise<TestItem[]>(
				async (
					resolve: (result: TestItem[]) => void,
				): Promise<void> => {
			...
			if (request.profile?.kind === TestRunProfileKind.Coverage) {
				...
				request.profile.loadDetailedCoverage = (
					...
					return Promise.resolve(
						coverageProvider!.getCoverageDetails(fileCoverage.uri),
					);
				};
			}
						...
						for (const testsInQueue of queue) {
							...
							for (const [
								...
								const testContext: IRunTestContext = {
									...
									testConfig: await loadRunConfig(
										itemsPerProject,
										workspaceFolder,
									),
								};
								...
								for (const [
									...
									try {
										...
										const resolvedConfiguration: DebugConfiguration =
											...
											(await resolveLaunchConfigurationForRunner(
												runner,
												testContext,
												testContext.testConfig,
											));
								...
								if (
									...
									await coverageProvider!.provideFileCoverage(
										run,
										projectName,
									);
								}
...
async function executeWithTestRunner(
	option: IRunOption,
	testRunner: TestRunner,
	testContext: IRunTestContext,
	run: TestRun,
	...
	await new Promise<void>(async (resolve: () => void): Promise<void> => {
		disposables.push(
			testRunner.onDidChangeTestItemStatus(
				(event: TestItemStatusChangeEvent) => {
					...
					if (
						event.displayName &&
						getLabelWithoutCodicon(currentItem.label) !==
							event.displayName
					) {
		...
		disposables.push(
			testRunner.onDidFinishTestRun((event: TestFinishEvent) => {
				if (event.statusCode === 2) {
					// See: https://build-server-protocol.github.io/docs/specification#statuscode
					window.showErrorMessage(
...
function mergeConfigurations(
	launchConfiguration: DebugConfiguration | undefined,
	config: any,
 ...
 * @returns
 */
async function getIncludedItems(
	request: TestRunRequest,
	token?: CancellationToken,
): Promise<TestItem[]> {
 ...
 */
export function handleInvocations(testItems: TestItem[]): TestItem[] {
	// export for unit test

	if (
		filterInvocations(testItems).some(
			(invocation: TestItem) =>
				!invocation.parent || !dataCache.get(invocation.parent),
		)
	) {
		// sanity-checks
		const errMsg: string =
...
function mergeInvocations(testItems: TestItem[]): TestItem[] {
	...
	testItems = testItems.filter(
		(item: TestItem) =>
			!(isInvocation(item) && isAncestorIncluded(item, testItems)),
	);

	// if all invocations of a method are selected, replace by single parent method run
	const invocationsPerMethod: Map<
		TestItem,
		Set<TestItem>
	> = filterInvocations(testItems)
		...
		.reduce(
			(map, inv) =>
				map.set(
					inv.parent!,
					map.has(inv.parent!)
						? new Set([...map.get(inv.parent!)!, inv])
						: new Set([inv]),
				),
			new Map(),
		);
	...
	return _.uniq(
		testItems.map((item: TestItem) =>
			invocationsToMerge.includes(item) ? item.parent! : item,
		),
	);
}

function isAncestorIncluded(
	item: TestItem,
	potentialAncestors: TestItem[],
): boolean {
...
function mergeTestMethods(testItems: TestItem[]): TestItem[][] {
	...
	// eslint-disable-next-line @typescript-eslint/typedef
	const testMapping: Map<TestItem, Set<TestItem>> = testItems.reduce(
		(map, i) => {
	...
	for (const [clazz, methods] of testMapping) {
		...
		if (
			clazz.children.size === methods.size &&
...
function trackTestFrameworkVersion(
	testKind: TestKind,
	classpaths: string[],
	modulepaths: string[],
) {
