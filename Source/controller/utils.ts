...
export async function loadJavaProjects(): Promise<void> {
	...
	if (
		testProjects.length === 1 &&
		testProjects[0].testKind === TestKind.None &&
 ...
 */
export function synchronizeItemsRecursively(
	parent: TestItem,
	childrenData: IJavaTestItem[] | undefined,
): void {
	if (childrenData) {
...
export function updateOrCreateTestItem(
	parent: TestItem,
 ...
 */
export async function updateItemForDocumentWithDebounce(
	uri: Uri,
 ...
 */
function findBelongingPackageItem(
...
export async function getJavaProjects(
	workspaceFolder: WorkspaceFolder,
	token?: CancellationToken,
): Promise<IJavaTestItem[]> {
	return (
		(await executeJavaLanguageServerCommand<IJavaTestItem[]>(
			JavaTestRunnerDelegateCommands.FIND_JAVA_PROJECTS,
...
export async function findTestPackagesAndTypes(
	handlerId: string,
	token?: CancellationToken,
): Promise<IJavaTestItem[]> {
	return (
...
export async function findDirectTestChildrenForClass(
	handlerId: string,
	token?: CancellationToken,
): Promise<IJavaTestItem[]> {
	return (
...
export async function findTestTypesAndMethods(
	uri: string,
	token?: CancellationToken,
): Promise<IJavaTestItem[]> {
	return (
		(await executeJavaLanguageServerCommand<IJavaTestItem[]>(
			JavaTestRunnerDelegateCommands.FIND_TEST_TYPES_AND_METHODS,
			uri,
			token,
		)) || []
	);
...
export async function resolvePath(uri: string): Promise<IJavaTestItem[]> {
	return (
		(await executeJavaLanguageServerCommand<IJavaTestItem[]>(
			JavaTestRunnerDelegateCommands.RESOLVE_PATH,
			uri,
		)) || []
	);
}
