...
import {
	registerAdvanceAskForChoice,
	registerAskForChoiceCommand,
	registerAskForInputCommand,
} from "./commands/askForOptionCommands";
...
import {
	refreshExplorer,
	runTestsFromTestExplorer,
} from "./commands/testExplorerCommands";
import { openStackTrace } from "./commands/testReportCommands";
import {
	Context,
	ExtensionName,
	JavaTestRunnerCommands,
	VSCodeCommands,
} from "./constants";
import {
...
} from "./provider/codeActionProvider";
		...
		replacementOptions: [
			{
				...
				replacementString:
					"Path must include project and resource name: /<REDACT>",
			},
		],
	});
...
function registerComponents(context: ExtensionContext): void {
	...
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			JavaTestRunnerCommands.JAVA_TEST_OPEN_STACKTRACE,
			openStackTrace,
		),
		instrumentOperationAsVsCodeCommand(
			...
			async () =>
				await commands.executeCommand(
					VSCodeCommands.RUN_TESTS_IN_CURRENT_FILE,
				),
		),
		instrumentOperationAsVsCodeCommand(
			JavaTestRunnerCommands.DEBUG_TEST_FROM_EDITOR,
			async () =>
				await commands.executeCommand(
					VSCodeCommands.DEBUG_TESTS_IN_CURRENT_FILE,
				),
		),
		instrumentOperationAsVsCodeCommand(
			JavaTestRunnerCommands.JAVA_TEST_GENERATE_TESTS,
			(uri: Uri, startPosition: number) =>
				generateTests(uri, startPosition),
		),
		instrumentOperationAsVsCodeCommand(
			JavaTestRunnerCommands.RUN_FROM_TEST_EXPLORER,
			async (node: TestItem, launchConfiguration: DebugConfiguration) =>
				await runTestsFromTestExplorer(
					node,
					launchConfiguration,
					false,
				),
		...
		instrumentOperationAsVsCodeCommand(
			JavaTestRunnerCommands.DEBUG_FROM_TEST_EXPLORER,
			async (node: TestItem, launchConfiguration: DebugConfiguration) =>
				await runTestsFromTestExplorer(
					node,
					launchConfiguration,
		...
		instrumentOperationAsVsCodeCommand(
			JavaTestRunnerCommands.RUN_TEST_FROM_JAVA_PROJECT_EXPLORER,
			async (node: any) =>
				await runTestsFromJavaProjectExplorer(
					node,
					false /* isDebug */,
				),
		),
		instrumentOperationAsVsCodeCommand(
			JavaTestRunnerCommands.DEBUG_TEST_FROM_JAVA_PROJECT_EXPLORER,
			async (node: any) =>
				await runTestsFromJavaProjectExplorer(node, true /* isDebug */),
		),
		instrumentOperationAsVsCodeCommand(
			JavaTestRunnerCommands.GO_TO_TEST,
			async () => await navigateToTestOrTarget(true),
		),
		instrumentOperationAsVsCodeCommand(
			JavaTestRunnerCommands.GO_TO_TEST_SUBJECT,
			async () => await navigateToTestOrTarget(false),
		),
		instrumentOperationAsVsCodeCommand(
			JavaTestRunnerCommands.ENABLE_TESTS,
			async () => await enableTests(),
		),
		...
		workspace.onDidChangeWorkspaceFolders(
			async (e: WorkspaceFoldersChangeEvent) => {
