...
export class JavaTestCoverageProvider {
	private coverageDetails: Map<Uri, FileCoverageDetail[]> = new Map<
		Uri,
		FileCoverageDetail[]
	>();

	public async provideFileCoverage(
		run: TestRun,
		projectName: string,
	): Promise<void> {
		const sourceFileCoverages: ISourceFileCoverage[] =
			(await executeJavaLanguageServerCommand<void>(
				JavaTestRunnerDelegateCommands.GET_COVERAGE_DETAIL,
				projectName,
