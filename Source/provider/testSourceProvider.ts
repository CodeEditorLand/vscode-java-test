...
class TestSourcePathProvider {
	...
	public async getTestSourcePattern(
	...
	public async getTestSourcePath(
		workspaceFolder: WorkspaceFolder,
		containsGeneral: boolean = true,
	): Promise<string[]> {
		...
		return testPaths
			.filter((s: ITestSourcePath) => s.isStrict)
...
async function getTestSourcePaths(uri: string[]): Promise<ITestSourcePath[]> {
	return (
		(await executeJavaLanguageServerCommand<ITestSourcePath[]>(
			JavaTestRunnerDelegateCommands.GET_TEST_SOURCE_PATH,
			uri,
		)) || []
	);
}
...
export const testSourceProvider: TestSourcePathProvider =
	new TestSourcePathProvider();
