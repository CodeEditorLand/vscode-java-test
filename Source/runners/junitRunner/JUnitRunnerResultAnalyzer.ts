...
export class JUnitRunnerResultAnalyzer extends RunnerResultAnalyzer {
	...
	public processData(data: string): void {
		if (data.startsWith(MessageId.TestTree)) {
			this.enlistToTestMapping(
				data.substring(MessageId.TestTree.length).trim(),
			);
			...
			setTestState(
				...
				item,
				this.getCurrentState(item).resultState,
			);
			...
			if (
				...
				this.assertionFailure = TestMessage.diff(
					`Expected [${this.expectString}] but was [${this.actualString}]`,
					this.expectString,
					this.actualString,
				);
			}
	...
	private determineResultStateOnFailure(
		data: string,
		currentState: CurrentItemState,
	): void {
	...
	private determineResultStateAtEnd(
		data: string,
		currentState: CurrentItemState,
	): void {
	...
	protected getTestId(message: string): string {
		if (
			message.includes("engine:junit5") ||
			message.includes("engine:junit-jupiter") ||
			message.includes("engine:jqwik")
		) {
	...
	protected getTestIdForJunit5Method(message: string): string {
		...
		parts.forEach((part: string) => {
			...
			if (part.startsWith(JUnitTestPart.CLASS)) {
				...
				const rawMethodName: string = part.substring(
					JUnitTestPart.METHOD.length,
				);
				...
				const rawMethodName: string = part.substring(
					JUnitTestPart.TEST_FACTORY.length,
				);
				// If the method name exists then we want to include the '#' qualifier.
				methodName = `#${this.getJUnit5MethodName(rawMethodName)}`;
				...
				const rawMethodName: string = part
					.substring(JUnitTestPart.TEST_TEMPLATE.length)
					.replace(/\\,/g, ",");
				// If the method name exists then we want to include the '#' qualifier.
				methodName = `#${this.getJUnit5MethodName(rawMethodName)}`;
			} else if (part.startsWith(JUnitTestPart.PROPERTY)) {
				const rawMethodName: string = part
					.substring(JUnitTestPart.PROPERTY.length)
					.replace(/\\,/g, ",");
				// If the method name exists then we want to include the '#' qualifier.
				methodName = `#${this.getJUnit5MethodName(rawMethodName)}`;
			} else if (
				part.startsWith(JUnitTestPart.TEST_TEMPLATE_INVOCATION)
			) {
	...
	protected getJUnit5MethodName(rawName: string): string {
		...
		let rawParamsString: string = rawName.substring(
			rawName.indexOf("(") + 1,
			rawName.indexOf(")"),
		);
		...
		rawParamsString = rawParamsString
			.replace(/\$/g, ".")
			.replace(/\\,/g, ",")
			.replace(/ /g, "");
			...
			if (isDynamic) {
				...
				if (parent) {
					...
					if (
						...
						testItem = this.enlistDynamicMethodToTestMapping(
							testId,
							parent,
							parentData,
							displayName,
				...
				if (testItem) {
					if (
						dataCache.get(testItem)?.testKind === TestKind.JUnit5 &&
						this.getLabelWithoutCodicon(testItem.label) !==
							displayName
					) {
	...
	public enlistDynamicMethodToTestMapping(
		testId: string,
		parent: TestItem,
		parentData: ITestItemData,
		displayName: string,
	...
	private async tryAppendMessage(
		item: TestItem,
