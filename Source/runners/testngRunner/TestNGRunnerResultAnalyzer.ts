...
export class TestNGRunnerResultAnalyzer extends RunnerResultAnalyzer {
	...
	public analyzeData(data: string): void {
		...
		while ((match = this.regex.exec(data)) !== null) {
			try {
				...
				this.testContext.testRun.appendOutput(
					`[ERROR] Failed to parse output data: ${match[1]}\n`,
				);
			}
		}
	}
	...
	public processData(data: string): void {
		const outputData: ITestNGOutputData = JSON.parse(
			data,
		) as ITestNGOutputData;

		this.testContext.testRun.appendOutput(
			this.unescape(data).replace(/\r?\n/g, "\r\n"),
		);
