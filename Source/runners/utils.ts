...
export async function findTestLocation(
	fullName: string,
): Promise<Location | undefined> {
	const location: any | undefined =
		await executeJavaLanguageServerCommand<any>(
 ...
 */
export function processStackTraceLine(
	lineOfMessage: string,
	traces: MarkdownString,
		...
		if (!sourceName || !lineNumLiteral) {
			...
			if (
				...
				if (currentItem.uri) {
					if (
						!currentItem.range ||
						(currentItem.range.start.line + 1 < lineNum &&
							currentItem.range.end.line + 1 > lineNum)
					) {
						testMessageLocation = new Location(
							currentItem.uri,
							new Range(lineNum - 1, 0, lineNum, 0),
						);
					} else {
						testMessageLocation = new Location(
							currentItem.uri,
							new Range(
								currentItem.range.start.line,
								0,
								currentItem.range.start.line,
								0,
							),
