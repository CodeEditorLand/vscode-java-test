...
export async function loadRunConfig(
...
): Promise<IExecutionConfig | undefined> {
	const configSetting: IExecutionConfig[] | IExecutionConfig = workspace
	...
	if (defaultConfigName) {
		...
		const defaultConfigs: IExecutionConfig[] = configItems.filter(
	...
	const candidateConfigItems: IExecutionConfig[] = filterCandidateConfigItems(
		configItems,
		testItems,
	);
...
function filterCandidateConfigItems(
	configItems: IExecutionConfig[],
	testItems: TestItem[],
): IExecutionConfig[] {
	return configItems.filter((config: IExecutionConfig) => {
		...
		if (whenClause) {
			const context: WhenClauseEvaluationContext =
				new WhenClauseEvaluationContext(whenClause);
...
function checkTestItems(
	testItems: TestItem[],
	context: WhenClauseEvaluationContext,
): boolean {
...
async function askPreferenceForConfig(
	configs: IExecutionConfig[],
	selectedConfig: IExecutionConfig,
...
export class WhenClauseEvaluationContext {
	...
	private tokenize(): Token[] {
		...
		const tokens: string[] = this.clause
	...
	private parse(token: string) {
		...
		if (!(token in this.context))
			throw new SyntaxError(
				`Context key not found in evaluation context: ${token}`,
			);
	...
	private evaluateTokens(tokens: Token[], start?: number, end?: number) {
		...
		while (currentTokens.length > 1) {
			const stringTokens: string[] = currentTokens.map(
				(token: Token) => token.stringValue,
			);
			...
			if (parenthesesEnd > parenthesesStart) {
				const resultToken: Token = this.evaluateTokens(
					currentTokens,
					parenthesesStart + 1,
					parenthesesEnd,
				);
			...
			for (const [operator, applyOperator] of Object.entries(
				WhenClauseEvaluationContext.OPERATORS,
			)) {
				const operatorIndex: number = currentTokens.findIndex(
					(token: Token) => token.stringValue === operator,
				);
