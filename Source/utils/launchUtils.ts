...
import {
	...
	Response,
} from "../runners/baseRunner/BaseRunner";
...
export async function resolveLaunchConfigurationForRunner(
	...
	const launchArguments: IJUnitLaunchArguments =
		await getLaunchArguments(testContext);

	if (config && config.vmArgs) {
		launchArguments.vmArguments.push(...config.vmArgs.filter(Boolean));
	} else if (config && (config as any).vmargs) {
		// to support the deprecated property name.
		launchArguments.vmArguments.push(
			...(config as any).vmargs.filter(Boolean),
		);
	...
	if (testContext.kind === TestKind.TestNG) {
		debugConfiguration = {
			...
			cwd:
				config && config.workingDirectory
					? config.workingDirectory
					: launchArguments.workingDirectory,
			classPaths: [
				...(config?.classPaths || []),
		...
		debugConfiguration = {
			...
			args: [
				...
				...(testContext.kind === TestKind.JUnit5
					? parseTags(config)
					: []),
			],
			vmArgs: launchArguments.vmArguments,
...
async function getLaunchArguments(
	testContext: IRunTestContext,
): Promise<IJUnitLaunchArguments> {
	const testLevel: TestLevel | undefined = dataCache.get(
		testContext.testItems[0],
	)?.testLevel;
	...
	const uniqueId: string | undefined =
		testContext.testItems.length === 1
			? dataCache.get(testContext.testItems[0])?.uniqueId
			: undefined;
...
function getTestNames(testContext: IRunTestContext): string[] {
	...
	if (
		dataCache.get(testContext.testItems[0])?.testLevel === TestLevel.Class
	) {
		return testContext.testItems
			.map((item: TestItem) => {
				return dataCache.get(item)?.fullName;
...
async function resolveJUnitLaunchArguments(
	projectName: string,
	testLevel: TestLevel,
	testKind: TestKind,
	testNames: string[],
	...
	const argument: Response<IJUnitLaunchArguments> | undefined =
		await executeJavaLanguageServerCommand<Response<IJUnitLaunchArguments>>(
			JavaTestRunnerDelegateCommands.RESOLVE_JUNIT_ARGUMENT,
			JSON.stringify({
				projectName,
				testLevel,
				testKind,
