...
copy(
	path.join(serverDir, "com.microsoft.java.test.plugin/target"),
	path.resolve("server"),
	(file) => path.extname(file) === ".jar",
);
copy(
	path.join(serverDir, "com.microsoft.java.test.runner/target"),
	path.resolve("server"),
	(file) => file.endsWith("jar-with-dependencies.jar"),
);
copy(
	path.join(
		serverDir,
		"com.microsoft.java.test.plugin.site/target/repository/plugins",
	),
	path.resolve("server"),
	(file) => {
...
function updateVersion() {
	...
	if (Array.isArray(javaExtensions)) {
		packageJsonData.contributes.javaExtensions = javaExtensions.map(
			(extensionString) => {
				const ind = extensionString.indexOf("_");
				const fileName = findNewRequiredJar(
					extensionString.substring(
						extensionString.lastIndexOf("/") + 1,
						ind,
					),
				);
				if (ind >= 0) {
